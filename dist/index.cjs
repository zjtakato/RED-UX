...
function useSelector(selector) {
    const { store } = React.useContext(StoreContext);
    const [, forceRender] = React.useReducer((_) => _ + 1, 0);
    const latestStoreState = React.useRef(store.getState());
    const latestSelectedState = React.useRef(selector(latestStoreState.current));
    React.useEffect(() => {
...
function connect(mapStateToProps, mapDispatchToProps) {
    ...
    return function currying(Component) {
        return function HOC(props) {
            ...
            const [, forceRender] = React.useReducer((_) => _ + 1, 0);
            const latestStoreState = React.useRef(store.getState());
            const latestSelectedState = React.useRef(mapStateToProps(latestStoreState.current));
            React.useEffect(() => {
                function checkUpdate() {
                    const newState = store.getState();
                    if (newState === latestStoreState)
